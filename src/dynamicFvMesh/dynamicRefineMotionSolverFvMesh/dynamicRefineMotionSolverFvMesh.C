...
bool Foam::dynamicRefineMotionSolverFvMesh::update()
{
    ...
    label refineIntervalRun = readLabel(refineDict.lookup("refineIntervalRun"));
    
    label refineIntervalSpinUp = readLabel(refineDict.lookup("refineIntervalSpinUp"));
    
    scalar spinUpDyM = readScalar(refineDict.lookup("spinUpDyM"));
    
    if (time().value() > spinUpDyM)
    {
        refineInterval = refineIntervalRun;
    }
    else
    {
        refineInterval = refineIntervalSpinUp;
    }*/
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
        ...
        const label nBufferLayers = readLabel(refineDict.lookup("nBufferLayers"));
        ...
        forAll(fieldNames,fieldi){
            ...
            //Info << "\nCurrent field : " << fieldNames[fieldi] << nl << endl;
            
            const word currentField = fieldNames[fieldi];
            const volScalarField& vFld = lookupObject<volScalarField>(currentField);
            const dictionary& subDict = refineDict.subDict(currentField);

            const scalar lowerRefineLevel = readScalar(subDict.lookup("lowerRefineLevel"));
            const scalar upperRefineLevel = readScalar(subDict.lookup("upperRefineLevel"));

            PackedBoolList candidateCell(nCells());
            {
                // Get error per cell. Is -1 (not to be refined) to >0 (to be refined,
                // higher more desirable to be refined).
                scalarField cellError
            ...
            /*if (fieldi == 0){
                refineCell = candidateCell;
            }else{
                forAll(candidateCell,celli){
                    if (refineCell[celli] == 0 && candidateCell[celli] == 1){
                        refineCell[celli] = 1;
                    }
                }
            }*/
        ...
        forAll(fieldNames, fieldi){
            ...
            const dictionary& subDict = refineDict.subDict(currentField);
            const scalar unrefineLevel = readScalar(subDict.lookup("unrefineLevel"));
            ...
            //label unrefI = returnReduce(pointsToUnrefine.size(), sumOp<label>());
            
            //if (fieldi == 0 || unrefI < unrefMin)
            //{
            //    unrefMin = unrefI;
            //}
        }
