...
bool Foam::dynamicRefineFvMesh2D::update()
{
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
        ...
        forAll(fieldNames, fieldi){
            ...
            if (fieldi == 0){
                unrefineCell = pointsToUnrefine;
            }else{
                unrefineCell.append(pointsToUnrefine);
            }        
        }

        // CL remove duplicates from unrefineCell
        // CL code based on : https://www.cfd-online.com/Forums/openfoam-programming-development/132353-how-merge-two-labellist.html
        labelList uniqueUnrefCell = unrefineCell;
        
        uniqueOrder(unrefineCell,uniqueUnrefCell);
        
        labelList unrefCell = uniqueUnrefCell;
        
        forAll(unrefCell,celli)
        {
            unrefCell[celli] = unrefineCell[uniqueUnrefCell[celli]];
        }
