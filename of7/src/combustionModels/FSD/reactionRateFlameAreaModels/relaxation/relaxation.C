...
:
    ...
    alpha1_
    (
        readScalar(dict.optionalSubDict(typeName + "Coeffs").lookup("alpha1"))
    ),
    alpha2_
    (
        readScalar(dict.optionalSubDict(typeName + "Coeffs").lookup("alpha2"))
    )
{}
...
)
{
    ...
    (
        alpha1_*sigma + alpha2_*turbulence.epsilon()/(turbulence.k() + kMin)
    );
...
)
{
    if (reactionRateFlameArea::read(dict))
    {
        ...
        coeffDict_.lookup("alpha1") >> alpha1_;
        coeffDict_.lookup("alpha2") >> alpha2_;
        correlation_.read
        (
