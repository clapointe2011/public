...
calculate()
{
    ...
    forAll(rho, celli)
    {
        ...
        if (this->reactingCells_[celli])
        {
            ...
            scalarField dcdt = omega(c, Ti, pi, true);
...
)
{
    ...
    scalarField& T = this->solidThermo().T();
    scalarField& p = this->solidThermo().p();
...
template<class CompType, class SolidThermo,class GasThermo>
Foam::tmp<Foam::volScalarField>
Foam::pyrolysisChemistryModel<CompType, SolidThermo, GasThermo>::gasRho
(
    const volScalarField& p,
    const volScalarField& T,
    const label index
) const
{
    ...
    (
        volScalarField::New
        (
            "rho_" + pyrolisisGases_[index],
            this->mesh_,
            dimensionedScalar(dimDensity, 0)
        )
    );

    volScalarField& gasRho = trho.ref();

    const GasThermo& mixture = gasThermo_[index];

    forAll(gasRho, celli)
    {
        gasRho[celli] = mixture.rho(p[celli], T[celli]);
    }
...
template<class CompType, class SolidThermo,class GasThermo>
Foam::tmp<Foam::volScalarField>
Foam::pyrolysisChemistryModel<CompType, SolidThermo, GasThermo>::gasMu
(
    const volScalarField& p,
    const volScalarField& T,
    const label index
) const
{
    ...
    (
        volScalarField::New
        (
            "mu_" + pyrolisisGases_[index],
            this->mesh_,
            dimensionedScalar(dimDensity*dimViscosity, 0)
        )
    );

    volScalarField& gasMu = tmu.ref();

    const GasThermo& mixture = gasThermo_[index];

    forAll(gasMu, celli)
    {
        gasMu[celli] = mixture.mu(p[celli], T[celli]);
    }
