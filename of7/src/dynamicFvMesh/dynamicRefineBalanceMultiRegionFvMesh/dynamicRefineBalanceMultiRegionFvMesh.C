...
namespace Foam
{
    defineTypeNameAndDebug(dynamicRefineBalanceMultiRegionFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicRefineBalanceMultiRegionFvMesh, IOobject);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::label Foam::dynamicRefineBalanceMultiRegionFvMesh::count
(
...
void Foam::dynamicRefineBalanceMultiRegionFvMesh::calculateProtectedCells
(
...
void Foam::dynamicRefineBalanceMultiRegionFvMesh::readDict()
{
    ...
    dumpLevel_ = Switch(refineDict.lookupOrDefault("dumpLevel", true));
}


// Refines cells, maps fields and recalculates (an approximate) flux
Foam::autoPtr<Foam::mapPolyMesh>
Foam::dynamicRefineBalanceMultiRegionFvMesh::refine
(
...
)
{
    ...
    //autoPtr<mapPolyMesh> map = meshMod.changeMesh(*this, false, true, true);
    ...
    */
    ...
    //CL test clearing/updating mapped patches
    /*const polyMesh& mesh(*this);
    polyBoundaryMesh& pbm = const_cast<polyBoundaryMesh&>(mesh.boundaryMesh());
    forAll(pbm, patchi)
    {
        if (pbm[patchi].type() == "mappedWall")
        {
            const mappedWallPolyPatch& pp = refCast<const mappedWallPolyPatch>(pbm[patchi]);
	    pp.AMI(true);
        }
    }*/
...
Foam::autoPtr<Foam::mapPolyMesh>
Foam::dynamicRefineBalanceMultiRegionFvMesh::unrefine
(
...
)
{
    ...
    //autoPtr<mapPolyMesh> map = meshMod.changeMesh(*this, true, true, true);
    autoPtr<mapPolyMesh> map = meshMod.changeMesh(*this, false);
...
Foam::scalarField
Foam::dynamicRefineBalanceMultiRegionFvMesh::maxPointField(const scalarField& pFld) const
{
...
Foam::scalarField
Foam::dynamicRefineBalanceMultiRegionFvMesh::maxCellField(const volScalarField& vFld) const
{
...
Foam::scalarField
Foam::dynamicRefineBalanceMultiRegionFvMesh::cellToPoint(const scalarField& vFld) const
{
...
Foam::scalarField Foam::dynamicRefineBalanceMultiRegionFvMesh::error
(
...
void Foam::dynamicRefineBalanceMultiRegionFvMesh::selectRefineCandidates
(
...
Foam::labelList Foam::dynamicRefineBalanceMultiRegionFvMesh::selectRefineCells
(
...
//CL getSplitElems to use hexRef from https://github.com/ElsevierSoftwareX/SOFTX_2018_143
Foam::labelList Foam::dynamicRefineBalanceMultiRegionFvMesh::selectUnrefinePoints
(
...
void Foam::dynamicRefineBalanceMultiRegionFvMesh::extendMarkedCells
(
...
void Foam::dynamicRefineBalanceMultiRegionFvMesh::checkEightAnchorPoints
(
...
Foam::dynamicRefineBalanceMultiRegionFvMesh::dynamicRefineBalanceMultiRegionFvMesh(const IOobject& io)
:
    dynamicFvMesh(io),
    meshCutter_(hexRef::New(*this)), //CL from https://github.com/ElsevierSoftwareX/SOFTX_2018_143
    dumpLevel_(false),
...
Foam::dynamicRefineBalanceMultiRegionFvMesh::~dynamicRefineBalanceMultiRegionFvMesh()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool Foam::dynamicRefineBalanceMultiRegionFvMesh::update()
{
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
        ...
        List<scalar> lowerRefineLevelList(fieldNames.size());
        List<scalar> upperRefineLevelList(fieldNames.size());
        List<scalar> unrefineLevelList(fieldNames.size());
        List<label> nBufferLayersList(fieldNames.size());
        List<label> maxRefinementList(fieldNames.size());

        //CL refine step
        PackedBoolList masterRefineCell(nCells());
        
        forAll(fieldNames,fieldi)
        {
            const word currentField = fieldNames[fieldi];
            const dictionary& fldDict = refineDict.subDict(currentField);
            lowerRefineLevelList[fieldi] = readScalar(fldDict.lookup("lowerRefineLevel"));
            upperRefineLevelList[fieldi] = readScalar(fldDict.lookup("upperRefineLevel"));
            unrefineLevelList[fieldi] = readScalar(fldDict.lookup("unrefineLevel"));
            nBufferLayersList[fieldi] = readLabel(fldDict.lookup("nBufferLayers"));
            maxRefinementList[fieldi] = readLabel(fldDict.lookup("maxRefinement"));
        }
        ...
        forAll(fieldNames,fieldi)
        {
            ...
            const word currentField = fieldNames[fieldi];
            const volScalarField& vFld = lookupObject<volScalarField>(currentField);

            const scalar lowerRefineLevel = lowerRefineLevelList[fieldi];
            const scalar upperRefineLevel = upperRefineLevelList[fieldi];
            const label nBufferLayers = nBufferLayersList[fieldi];
            const label maxRefinement = maxRefinementList[fieldi];
	    ...
	    masterRefineCell |= refineCell;
        }
        
        // CL unrefine step
        DynamicList<label> unrefineCell;
        
        bool zeroUnrefinePoints = false;
        
        forAll(fieldNames, fieldi)
        {    
            const word currentField = fieldNames[fieldi];
            const volScalarField& vFld = lookupObject<volScalarField>(currentField);
            const scalar unrefineLevel = unrefineLevelList[fieldi];

            // Select unrefineable points that are not marked in refineCell
            labelList pointsToUnrefine
            (
                //CL to work with github.com/ElsevierSoftwareX/SOFTX_2018_143
            ...
            unrefineCell.append(pointsToUnrefine);
            
            label pointsToUnrefineSize = returnReduce(pointsToUnrefine.size(),sumOp<label>());
            
            //duplicatePoints apparently has trouble with zero size lists, so track zeros as a workaround
            if (pointsToUnrefineSize == 0)
            {
                zeroUnrefinePoints = true;
            }
        }
        
        label nSplitUnrefCellTot= returnReduce
        (
            unrefineCell.size(),
            sumOp<label>()
        );
        
        Info << "Selected " << nSplitUnrefCellTot << " total split points." << endl;
        ...
        if (fieldNames.size() > 1)
        {
            for (int i = 0; i < fieldNames.size() - 1; i++)
            {
                ...
                duplicateOrder(unrefineCell,duplicates);
                
                label nDuplicates = returnReduce(duplicates.size(),sumOp<label>());
            
                if (nDuplicates > 0)
                {
                    DynamicList<label> unrefCell;
                    
                    forAll(duplicates,indexi)
                    {
                        unrefCell.append(unrefineCell[duplicates[indexi]]);
                    }
                
                    unrefineCell.clear();
                    unrefineCell = unrefCell;
                }
            }
        }
        
        if (zeroUnrefinePoints)
        {
            unrefineCell.clear();
        }
            
        label nSplitUnrefCellMutual= returnReduce
        (
            unrefineCell.size(),
            sumOp<label>()
        );
            
        Info << "Selected " << nSplitUnrefCellMutual << " mutual split points." << endl;

        {
            if (nSplitUnrefCellMutual > 0)
            {
                // Refine/update mesh
                unrefine(unrefineCell);
...
bool Foam::dynamicRefineBalanceMultiRegionFvMesh::redistribute()
{
    if (!Pstream::parRun())
    {
        return 0;
    ...
    if (Pstream::master())
    {
        Info << "Balancing mesh..." << nl << endl;
        Foam::system("myReconstructParMesh -allRegions >> log.balance");
        Foam::system("reconstructPar -allRegions >> log.balance");
        Foam::system("rm -r processor*");
        Foam::system("decomposePar -latestTime -allRegions -force >> log.balance");
        Foam::system("rm log.balance");
    }

    return false;
}

bool Foam::dynamicRefineBalanceMultiRegionFvMesh::balance()
{
    if (!Pstream::parRun())
    {
        return false;
    ...
    const scalar allowableImbalance =
        dynamicMeshDict.lookupOrDefault("allowableImbalance", 0.2);
    ...
    Info <<"\nMaximum imbalance = " << 100*maxImbalance << " %" <<endl;
...
bool Foam::dynamicRefineBalanceMultiRegionFvMesh::writeObject
(
